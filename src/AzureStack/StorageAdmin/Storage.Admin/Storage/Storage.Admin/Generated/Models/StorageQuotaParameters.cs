// <auto-generated>
// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See License.txt in the project root for
// license information.
//
// Code generated by Microsoft (R) AutoRest Code Generator.
// Changes may cause incorrect behavior and will be lost if the code is
// regenerated.
// </auto-generated>

namespace Microsoft.AzureStack.Management.Storage.Admin.Models
{
    using Microsoft.Rest;
    using Microsoft.Rest.Serialization;
    using Newtonsoft.Json;
    using System.Linq;

    [Rest.Serialization.JsonTransformation]
    public partial class StorageQuotaParameters
    {
        /// <summary>
        /// Initializes a new instance of the StorageQuotaParameters class.
        /// </summary>
        public StorageQuotaParameters()
        {
            CustomInit();
        }

        /// <summary>
        /// Initializes a new instance of the StorageQuotaParameters class.
        /// </summary>
        public StorageQuotaParameters(int? numberOfStorageAccounts = default(int?), int? capacityInGb = default(int?))
        {
            NumberOfStorageAccounts = numberOfStorageAccounts;
            CapacityInGb = capacityInGb;
            CustomInit();
        }

        /// <summary>
        /// An initialization method that performs custom operations like setting defaults
        /// </summary>
        partial void CustomInit();

        /// <summary>
        /// </summary>
        [JsonProperty(PropertyName = "properties.numberOfStorageAccounts")]
        public int? NumberOfStorageAccounts { get; set; }

        /// <summary>
        /// </summary>
        [JsonProperty(PropertyName = "properties.capacityInGb")]
        public int? CapacityInGb { get; set; }

    }
}
